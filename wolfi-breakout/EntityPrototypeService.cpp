...
void EntityPrototypeService::OnStartup()
{
  ...
  entityFactory.RegisterPrototype("BottomBoundary", CreateBoundary(VIEW_WIDTH, BOUNDARY_THICKNESS, true));
...
shared_ptr<Entity> EntityPrototypeService::CreateBoundary(float w, float h, bool lethal)
{
...
shared_ptr<Entity> EntityPrototypeService::CreatePaddle() {
  ...
  entity->AddComponent(make_shared<CScene>(PolylineBuilder()
      .Color(PADDLE_COLOR)
      .VertexBuffer(ShapeGenerator().GenRectangle(PADDLE_WIDTH, PADDLE_HEIGHT))
      .Build()));
  ...
  float maxX = ((VIEW_WIDTH / 2) - PADDLE_WIDTH / 2) - 0.01f;   // -0.01f to keep all pixels on screen.
...
shared_ptr<Entity> EntityPrototypeService::CreateBrick(int hits, Color4f color)
{
  auto entity = make_shared<Entity>();
  entity->AddComponent(make_shared<CPose>());
  ...
  entity->AddComponent(make_shared<CScene>(PolylineBuilder()
      .Color(color)
      .Name("Brick")
      .VertexBuffer(ShapeGenerator().GenRectangle(BRICK_WIDTH, BRICK_HEIGHT))
      .Build()));
